...
player = turntable.TurnTable(os.path.join(os.path.expanduser("~"), "Music"))
player.play()

while True:
    if buttonB.value and not buttonA.value:  # just button A pressed
        if backlight.value: backlight.value = False  # turn off backlight
        else: backlight.value = True  # turn on backlight
    elif buttonA.value and not buttonB.value:  # just button B pressed
        del player
        player = turntable.TurnTable("Music")
        player.play()
    elif not buttonA.value and not buttonB.value:  # both pressed
        pass

    # Get track playing information
    POS = player.get_prog(16, ' ', '_')
    ...
    '''
    interf.name = NAME
    interf.show_text((ART, ALB, VOL, TIME), (6, 2, 1, 5))
    image = interf.image
    '''
